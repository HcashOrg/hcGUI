...
import * as pi from "../httpService/server/autonomyApi";
...
// enum values from autonomywww's v1.PropVoteStatusT
...
const fillVotes = async (proposal, propVotes, walletService) => {
  ...
  propVotes.data.castvotes.some(vote => {
    ...
    if (!choiceID) { throw "ERROR: choiceID not found on vote", vote; }
...
export const getVettedProposals = (after, callBack) => async (dispatch, getState) => {
  ...
  const lastAccessTime = cfg.get("autonomy_last_access_time") || 0;
  const lastAccessBlock = cfg.get("autonomy_last_access_block") || 0;
  ...
  cfg.set("autonomy_last_access_time", (new Date()).getTime());
  cfg.set("autonomy_last_access_block", currentBlockHeight);
};
